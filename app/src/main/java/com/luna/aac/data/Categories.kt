package com.luna.aac.data

import com.luna.aac.R

val categoryItems = listOf(
    Category(R.drawable.ic_launcher_background, "dummy", emptyList()),
    Category(R.drawable.general_0, "일상", generalExpressions),
    Category(R.drawable.help_0, "도움, \n 요구하기", helpExpressions),
    Category(R.drawable.answer_0, "대답하기", emptyList()),
    Category(R.drawable.ask_0, "질문하기", emptyList()),
    Category(R.drawable.feeling_0, "감정", emptyList()),
    Category(R.drawable.body_0, "신체", emptyList()),
    Category(R.drawable.food_0, "음식", emptyList()),
    Category(R.drawable.useful_0, "생활용품", emptyList()),
    Category(R.drawable.place_0, "장소", emptyList()),
    Category(R.drawable.number_0, "번호", emptyList()),
)
